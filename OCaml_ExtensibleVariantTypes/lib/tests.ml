...
let parse_test s expected_res =
  List.equal Ast.equal_decl expected_res (Parser.parse_program s)
;;

let%test _ =
  parse_test
    "let a = 5 + 6"
    [ ( DLet (Not_recursive, Ident "a", EParams [])
      , EBinop (EConst (CInt 5), Add, EConst (CInt 6)) )
    ]
;;
...
let%test _ =
  parse_test
    ...
    [ ( DLet (Recursive, Ident "f", EParams [ EId (Ident "n") ])
      , EIf
          ...
          , EBinop
              ( EId (Ident "n")
              , Mul
              , EApp (EId (Ident "f"), EBinop (EId (Ident "n"), Sub, EConst (CInt 1))) )
          ) )
    ]
;;
...
let%test _ =
  parse_test
    "let f x y = g x + g y"
    [ ( DLet (Not_recursive, Ident "f", EParams [ EId (Ident "x"); EId (Ident "y") ])
      , EBinop
...
let%test _ =
  parse_test
    ...
    [ ( DLet (Not_recursive, Ident "g", EParams [ EId (Ident "x"); EId (Ident "y") ])
      , EBinop
          ( EApp
              ( EApp (EApp (EId (Ident "f"), EId (Ident "x1")), EId (Ident "x2"))
              , EBinop (EId (Ident "x3"), Mul, EId (Ident "x3")) )
          , Add
          , EApp
              ( EApp (EApp (EId (Ident "f"), EId (Ident "x3")), EId (Ident "x2"))
              , EId (Ident "x1") ) ) )
    ]
;;
...
let%test _ =
  parse_test
    ...
    [ ( DLet (Not_recursive, Ident "a", EParams [])
      , EApp
          ( EAnonFun
              (EParams [ EId (Ident "k") ], EBinop (EId (Ident "k"), Add, EConst (CInt 2)))
          , EConst (CInt 3) ) )
    ]
;;
...
let%test _ =
  parse_test
    "let s = -f 3"
    [ ( DLet (Not_recursive, Ident "s", EParams [])
      , EUnop (Minus, EApp (EId (Ident "f"), EConst (CInt 3))) )
    ]
;;
...
let%test _ =
  parse_test
    "let s = f -3"
    [ ( DLet (Not_recursive, Ident "s", EParams [])
      , EBinop (EId (Ident "f"), Sub, EConst (CInt 3)) )
    ]
;;
...
let%test _ =
  parse_test
    "let s = f (-3)"
    [ ( DLet (Not_recursive, Ident "s", EParams [])
      , EApp (EId (Ident "f"), EConst (CInt (-3))) )
    ]
;;
